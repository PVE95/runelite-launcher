...
APPBASE="build/macos-aarch64/LuzoxPK.app"

build() {
    echo Launcher sha256sum
    shasum -a 256 build/libs/LuzoxPK.jar
    ...
    cp native/build-aarch64/src/LuzoxPK $APPBASE/Contents/MacOS/
    cp build/libs/LuzoxPK.jar $APPBASE/Contents/Resources/
    ...
    echo Setting world execute permissions on LuzoxPK
    pushd $APPBASE
    chmod g+x,o+x Contents/MacOS/LuzoxPK
    popd

    otool -l $APPBASE/Contents/MacOS/LuzoxPK
}
...
dmg() {
    ...
    mv LuzoxPK\ *.dmg LuzoxPK-aarch64.dmg

    # dump for CI
    hdiutil imageinfo LuzoxPK-aarch64.dmg

    if ! hdiutil imageinfo LuzoxPK-aarch64.dmg | grep -q "Format: ULFO" ; then
        echo Format of dmg is not ULFO
        exit 1
    fi

    if ! hdiutil imageinfo LuzoxPK-aarch64.dmg | grep -q "Apple_HFS" ; then
        echo Filesystem of dmg is not Apple_HFS
        exit 1
    fi

    # Notarize app
    if xcrun notarytool submit LuzoxPK-aarch64.dmg --wait --keychain-profile "AC_PASSWORD" ; then
        xcrun stapler staple LuzoxPK-aarch64.dmg
    fi
}
